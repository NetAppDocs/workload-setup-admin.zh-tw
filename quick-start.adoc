...
[role="quick-margin-para"]
此步驟為可選步驟。您無需新增憑證即可使用 Workload Factory 存取您的 AWS 帳戶。將 AWS 憑證新增至 Workload Factory 後，您的 Workload Factory 帳戶將擁有建立和管理 FSx for ONTAP 檔案系統以及部署和管理特定工作負載（例如資料庫）所需的權限。
