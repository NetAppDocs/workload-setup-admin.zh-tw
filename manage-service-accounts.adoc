...
. 您也可以透過執行客戶端憑證交換來取得 Auth0 管理 API 的存取權杖。有關產生刷新令牌的資訊，請參閱 link:https://docs.netapp.com/us-en/console-automation/platform/grant_types.html#refresh-token["NetApp Console API 文件"^]。
+
curl 範例展示如何使用客戶端 ID 和金鑰，透過 API 產生有時效性的存取權杖。該令牌提供數小時的 NetApp Workload Factory API 存取權限。
...
----
+
將輸出結果發佈到 \https://netapp-cloud-account.auth0.com/oauth/token 以取得存取權杖。
