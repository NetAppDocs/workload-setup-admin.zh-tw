...
.步驟
. 使用 UI 定義您要執行的作業之後、請選取 image:button-copy-codebox.png["複製按鈕"] 以複製 Codebox 中的程式碼。
...
.步驟
. 使用 UI 定義您要執行的作業之後、請選取 image:button-copy-codebox.png["複製按鈕"] 以複製 Codebox 中的 API 程式碼。
. 貼上程式碼、然後在主機系統上執行。
...
.步驟
. 使用 UI 來定義您要執行的作業之後、請在「代碼」方塊中選取 image:button-copy-codebox.png["複製按鈕"] 複製 AWS CLI 。
. 開啟另一個瀏覽器視窗、然後登入 AWS 管理主控台。
. 貼上程式碼並執行。
