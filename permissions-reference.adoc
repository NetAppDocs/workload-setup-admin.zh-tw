...
當您授予權限時，Workload Factory 會將政策附加到執行個體，賦予其管理該 AWS 帳戶內資源和程序的權限。這使得 Workload Factory 能夠執行各種操作，從探索您的儲存環境到部署 AWS 資源（例如儲存管理中的檔案系統）。
...
NOTE: AWS GovCloud 不支援 AWS Cost Explorer，因此您不需要這些權限： `ce:GetCostAndUsage` 和 `ce:GetTags`。如果您包含這些權限，則該原則仍然有效。
...
| 取得 AWS CloudTrail 追蹤的目前事件選擇器組態 | cloudtrail:GetEventSelectors | S3 存取點日誌操作 | 營運和補救 


| 透過啟用或停用 FSx for ONTAP S3 存取點與 CloudTrail 追蹤之間的關聯，設定或更新 AWS CloudTrail 事件選擇器 | cloudtrail：PutEventSelectors | S3 存取點日誌操作 | 營運和補救 
|===
====
...
| 從已註冊的 SQL 節點收集已儲存至 Amazon CloudWatch 的效能指標。資料將在已註冊 SQL 實例的管理實例畫面上產生效能趨勢圖。 | cloudswatch ： GetMetricData | 庫存 | 營運和補救 
...
| 部署後發送管理操作命令後，輪詢 SSM 回應 | SSM) ： GetCommandInvocation  a| 
* 管理作業
* 庫存
* 探索節約效益
* 最佳化
...
| 取得 Windows EC2 執行個體的修補程式清單詳細資訊，以進行評估和修復工作流程。 | ssm:DescribeInstancePatches | 最佳化 | 視圖、規劃與分析 


| 取得可用於作業系統修補程式評估的修補程式清單 | ssm:DescribeAvailablePatches | 最佳化 | 視圖、規劃與分析 


| 從代管 SQL 節點擷取收集的庫存中繼資料 | ssm:GetInventory | 庫存 | 視圖、規劃與分析 


| 列出從代管 SQL 節點收集的庫存條目 | ssm:ListInventoryEntries | 庫存 | 視圖、規劃與分析 
...
| 取得可用於錯誤日誌分析的基礎模型 | Bedrock:GetFoundationModelAvailability | 錯誤日誌分析 | 營運和補救 


| 列出 Amazon Bedrock 中可用於錯誤日誌分析的介面設定檔 | Bedrock ： ListInferenceProfiles | 錯誤日誌分析 | 營運和補救 
|===
. 權限僅限於從 WLMDB 開始的資源。
...
操作和修復權限可讓您進行全面的架構完善評估、根據架構完善分析套用建議的動作、變更您的資源，以及分析錯誤日誌。
