...
了解更多關於每個工作負載功能所需的權限的資訊，請參閱 link:permissions-reference.html["權限參考"]。
...
. 從選單中選擇“*管理*”，然後選擇“*憑證*”。
. 在「認證」頁面上，選取 * 新增認證 * 。
. 在「新增憑證」頁面上，選擇「手動新增」，然後按照下列步驟完成「權限組態」下的每個部分。
+
...
. 對於那些提供權限策略選擇的工作負載功能，請選擇使用這些憑證可取得的權限類型。
+

NOTE: 如果您已有 GovCloud 帳戶，則無需 Cost Explorer 權限 `ce:GetCostAndUsage`或 `ce:GetTags`在「儲存」的「檢視、規劃和分析」原則中擁有這些權限。從程式碼方塊複製腳本後，請移除這些權限並刪除上一行結尾的逗號。
...
.步驟
. 在 Workload Factory 的「新增憑證」頁面中，展開「權限組態」下的「建立角色」。
. 選擇 *JSON code* 以複製原則。
. 在 AWS 管理主控台中、選取 * 角色 > 建立角色 * 。
. 在「受信任實體類型」下，選擇「自訂信任原則」。
. 將文字方塊中的目前內容替換為您在步驟 2 中複製的 JSON 程式碼。
. 選擇*下一步*。
. 在權限原則區段中、選擇您先前定義的所有原則、然後選取 * 下一步 * 。
. 輸入角色名稱、然後選取 * 建立角色 * 。
. 複製角色 ARN 。
. 返回 Workload Factory 中的「新增憑證」頁面，然後將角色 ARN 貼到 _Role ARN_ 欄位中。
...
NOTE: 使用 CloudFormation 新增認證至 AWS GovCloud（US）帳戶不受支援。
